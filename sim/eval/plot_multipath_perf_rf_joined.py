...
for x in itertools.product(mappings, message_length, delay_range):
    filename = "".join("perf-data/multipath-perf-rf/results_{0}_{1}_{2}_.csv".format(*x))
...
for x in itertools.product(bit_rate, protocol, delay_range):
    filename = "".join("perf-data-ax25/multipath-perf-rf/results_{0}_{1}_{2}_.csv".format(*x))
    with open(filename) as file:
        errors = 0;
        for line in file:
            if "0" in line:
...
for j in range(0, len(mappings)+1):
    plt.plot(delay_range, 1 - error_curve[j, :], color = colors[j], label= mapping_str[j])
plt.xlabel("Path difference " + r'$T_{\Delta}$' +" in ms")
plt.ylabel("Packet Reception Rate")
plt.title("FM Multipath Error Rates")
plt.ylim(-0.05, 1.05)
plt.legend(loc='upper right')
plt.savefig('res/multipath_rf.pdf')
plt.show()
