...
for x in itertools.product(mappings, message_length, snr_read):
    filename = "".join("perf-data/snr-perf-rf-new/results_{0}_{1}_{2}_.csv".format(*x))
...
for x in itertools.product(bit_rate, protocol, snr_read):
    filename = "".join("perf-data-ax25/snr-perf-rf-new/results_{0}_{1}_{2}_.csv".format(*x))
    with open(filename) as file:
        errors = 0;
        for line in file:
            if "0" in line:
...
for i in range(0, len(mappings)+1):
    err = gaussian_filter1d(1 - error_curve[i, :], sigma=1)
    curve[20:81] = err[0:61]
    plt.plot(snr_range, curve, color = colors[i], label= str(mapping_str[i]))
plt.xlabel("SNR in dB")
plt.ylabel("Packet Reception Rate")
plt.title("FM SNR Error Rates")
plt.ylim(-0.05, 1.05)
plt.legend(loc='upper right')
plt.savefig('res/snr_rf.pdf')
plt.show()