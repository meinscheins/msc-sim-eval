...
for x in itertools.product(mappings, message_length, cfo_range):
    filename = "".join("perf-data/cfo-perf-acoustic/results_{0}_{1}_{2}_.csv".format(*x))
...
for x in itertools.product(bit_rate, protocol, cfo_range):
    filename = "".join("perf-data-ax25/cfo-perf-acoustic/results_{0}_{1}_{2}_.csv".format(*x))
    with open(filename) as file:
        errors = 0;
        for line in file:
            if "0" in line:
...
plt.savefig('res/cfo_acoustic.pdf')
plt.show()
